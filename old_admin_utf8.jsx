// ============================================================
// AdminPages.jsx ÔÇö Dashboard Admin
...
// ÔöÇÔöÇ Helper export Excel ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  return (
    <div className="fade-in">
      ...
      <div className="stats-grid">
        <StatCard icon="­ƒæ¿ÔÇì­ƒÄô" label="Siswa Aktif"        value={aktif}                                          bgColor="#dbeafe" textColor="#2563eb" />
        <StatCard icon="­ƒæ¿ÔÇì­ƒÅ½" label="Total Guru"          value={TEACHERS_DATA.length}                           bgColor="#dcfce7" textColor="#16a34a" />
        <StatCard icon="Ô£à"   label="SPP Sudah Bayar"    value={sudahBayar + " siswa"}                          bgColor="#dcfce7" textColor="#16a34a" />
        <StatCard icon="ÔÅ│"   label="SPP Belum Bayar"    value={belumBayar + " siswa"}                          bgColor="#fee2e2" textColor="#dc2626" />
      </div>

      {/* Stats row 2 */}
      <div className="stats-grid" style={{ marginTop: 0 }}>
        <StatCard icon="­ƒÆÁ" label="Total Pemasukan"   value={"Rp " + totalMasuk.toLocaleString("id-ID")}  bgColor="#dcfce7" textColor="#16a34a" />
        <StatCard icon="­ƒÆ©" label="Total Pengeluaran" value={"Rp " + totalKeluar.toLocaleString("id-ID")} bgColor="#fee2e2" textColor="#dc2626" />
        <StatCard icon="­ƒÅª" label="Saldo Saat Ini"    value={"Rp " + saldo.toLocaleString("id-ID")}       bgColor="#dbeafe" textColor="#2563eb" />
        <StatCard icon="­ƒôØ" label="Total Artikel"     value={ARTICLES.length}                             bgColor="#f3e8ff" textColor="#7c3aed" />
      </div>

      <div className="content-grid-2">
        {/* Siswa terbaru */}
        <div className="table-card">
          <div className="table-head"><h3>­ƒæ¿ÔÇì­ƒÄô Siswa Terbaru</h3></div>
          <table>
        ...
        <div className="content-card">
          <h3 style={{ marginBottom: 14 }}>­ƒôè Ringkasan Bulan Ini</h3>
          {[
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        <input className="form-input" style={{ maxWidth: 260 }} placeholder="­ƒöì Cari nama / sekolah / program..."
        ...
        <div style={{
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 560, maxHeight: "80vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3>Rincian SPP ÔÇö {siswaRincian.nama}</h3>
              <button onClick={() => setRincianId(null)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1.2rem" }}>Ô£ò</button>
            </div>
...
export function ManajemenGuru() {
  ...
  return (
    <div className="fade-in">
      {/* Toolbar */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        <input className="form-input" style={{ maxWidth: 260 }} placeholder="­ƒöì Cari nama / email..."
          value={search} onChange={e => setSearch(e.target.value)} />
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn-outline" style={{ padding: "9px 14px", fontSize: ".82rem" }} onClick={handleExport}>
            ­ƒôÑ Export Excel
      ...
      {showForm && (
        <div className="content-card" style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            <button className="btn-primary" style={{ padding: "10px 18px", fontSize: ".85rem" }} onClick={handleSave}>
              ­ƒÜÇ {editId ? "Update" : "Publikasikan"}
            </button>
            <button className="btn-outline" style={{ padding: "10px 18px", fontSize: ".85rem" }} onClick={resetForm}>Batal</button>
          </div>
        </div>
            ...
            <tbody>
              {articles.map(a => (
                <tr key={a.id}>
                  ...
                  <td>
                    {a.video_url
                      ? <span style={{ fontSize: ".75rem", color: "#16a34a", fontWeight: 700 }}>ÔûÂ Ada</span>
                      : <span style={{ fontSize: ".75rem", color: "var(--muted)" }}>ÔÇö</span>}
                  </td>
      ...
      {showForm && (
        <div className="content-card" style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button className="btn-primary" style={{ padding: "10px 18px", fontSize: ".85rem" }} onClick={handleAdd}>­ƒÆ¥ Simpan</button>
...
export function SettingHonor() {
  ...
  // ÔöÇÔöÇ Honor per Program ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Komponen Tetap ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 20 }}>
        ...
        {/* ÔöÇÔöÇ Bagian 1: Honor per Program ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        <div>
          <div className="table-card">
            <div className="table-head">
              <h3>­ƒôÜ Honor per Program</h3>
        ...
        {/* ÔöÇÔöÇ Bagian 2: Komponen Tetap ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        <div>
          <div className="table-card">
            <div className="table-head">
              <h3>­ƒÆ░ Komponen Tetap</h3>
              <button className="btn-primary" style={{ padding: "7px 12px", fontSize: ".78rem" }}
              ...
              <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)", background: "#f8fafc" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn-primary" style={{ padding: "7px 14px", fontSize: ".8rem" }} onClick={handleSaveKomponen}>
                    ­ƒÆ¥ {editKomponenId ? "Update" : "Simpan"}
                  </button>
          ...
          <div style={{
            ...
            ­ƒÆí <strong>Catatan:</strong> Nominal default adalah nilai awal saat input honor bulan baru.
    ...
    <div className="fade-in">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        <input className="form-input" style={{ maxWidth: 260 }}
          placeholder="­ƒöì Cari program / jenjang..."
          value={search} onChange={e => setSearch(e.target.value)} />
      ...
      {showForm && (
        <div className="content-card" style={{ marginBottom: 20 }}>
          ...
          <div className="info-box" style={{ marginTop: 10 }}>
            ­ƒÆí Biaya SPP tidak diisi di sini ÔÇö nominal SPP diinput per siswa karena bisa berbeda tiap siswa.
          </div>
      ...
      <div className="table-card">
        ...
        <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border)" }}>
          <div className="info-box">
            ­ƒÆí Program yang ditambahkan di sini otomatis tersedia di <strong>Data Siswa</strong> dan <strong>Setting Honor Guru</strong>.
          </div>
        </div>
      </div>
    </div>
